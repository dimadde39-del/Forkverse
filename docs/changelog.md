...
- `prompts/parser_v1.txt` заново зафиксирован в сверхжёстком JSON-режиме: без новых ключей, без формул и без отклонений от `ready/needs_clarification` структуры; `api/parse.py` дополнительно сверен на наличие `response_format: {"type": "json_object"}` для Groq.
- Синхронизированы ключи parse/simulate/frontend: `components/SimulatorClient.tsx` читает симуляционные поля прямо из `parseEnvelope.data`, `api/parse.py` возвращает top-level `months`, `n_simulations`, `survival_probability`, `p10`, `p50`, `p90`, `spaghetti_sample`, а `api/simulate.py` снова лениво подключает реальный движок вместо `Simulation engine unavailable`.
- `api/parse.py` обновлён до `llama-3.3-70b-versatile`, а `prompts/parser_v1.txt` ужесточён: жёсткий JSON-only режим, фиксированное `n_simulations = 100`, суммирование расходов только в одно число `monthly_burn`.
- `_run_simulation` в `api/parse.py` переписан по-настоящему: независимый multiplicative noise `N(1.0, 0.15)` применяется отдельно к `monthly_income` и `monthly_burn`, а после первого месяца с `capital <= 0` траектория переводится в absorbing barrier и остаётся на нуле до конца горизонта.
